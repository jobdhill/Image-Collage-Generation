...
def build_palette(uploaded_files, status_placeholder):
    ...
    for i, f in enumerate(uploaded_files):
        ...
        try:
            ...
            not_working.append((f.name, str(e)))
            try:
                os.remove(tmp_path)
            except OSError:
                pass
    ...
    if not_working:
        st.warning(
            f"⚠️ {len(not_working)} source image(s) could not be processed: "
            + ", ".join(name for name, _ in not_working)
        )
...
def render_collage(
    ...
    cropped_width = tile_width * num_cols
    cropped_height = tile_width * num_rows
    target = target.crop((0, 0, cropped_width, cropped_height))

    mosaic = Image.new("RGB", (cropped_width, cropped_height))
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            tile = target.crop((x, y, x + tile_width, y + tile_height))
            avg_color = tuple(np.array(tile).mean(axis=(0, 1)).astype(int))

            best_path = palette.find_closest_match(avg_color)

            # Load source image from its saved filepath
            src = Image.open(best_path.filepath).convert("RGB")
            src = src.resize((tile_width, tile_height), Image.Resampling.LANCZOS)
