...
def get_target_image_path(config: Dict[str, Any]) -> str:
    ...
    target_directory = config['target_images']['directory']
    ...
    if len(target_files) == 1:
        target_path = os.path.join(target_directory, target_files[0])
        print(f"Using {target_files[0]} as the target file")
        return target_path
    ...
    while True:
        try:
            ...
            target_path = os.path.join(target_directory, target_files[idx])
...
def create_collage(target_image_path: str, palette: SourceImagePalette, 
    ...
    collage = render_collage(target_image=target_image, palette=palette, tile_size=tile_size, method="euclidean")
...
def main():
    ...
    try:
        ...
        target_path = get_target_image_path(config)

        
        
        # Step 6: Create the collage
        collage = create_collage(target_path, palette,config)

        
        
        # Step 7: Save the result
        output_path = save_collage(collage, target_path,config)
